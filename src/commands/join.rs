use serenity::{prelude::{Context}, framework::standard::CommandResult, model::channel, async_trait};
use serenity::model::prelude::Message;
use songbird::{TrackEvent, EventHandler, EventContext, Event};
...
async fn join(context: &Context, msg: &Message) -> CommandResult {
    ...
    let manager = songbird::get(context).await.expect("Could not retrieve songbird client").clone();
